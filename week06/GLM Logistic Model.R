###### This exercise is and old one but a good one.    ######
##### The first part is due to Josh Stormer at UNC     ######
##### Chapel Hill who was kind enough to share this    ######
##### with me a few years ago.  I have add the train   ######
##### test exercise at the end.                        ######
##### The code illustrates Logistic Regression with    ######
...
#### Get the data directly from the site
url <- "http://archive.ics.uci.edu/ml/machine-learning-databases/heart-disease/processed.cleveland.data"

data <- read.csv(url, header=FALSE)

######  The data will beed to clearned.  Follow the following code lines:
##
...
head(data) # We need to name the columns.  Explanations are included:
...
#### Let's impliment the Train/Test approach.

set.seed(1200)
TrainIDs <- sample(nrow(data), 0.70*nrow(data), replace=FALSE)
Train <- data[TrainIDs,]
Test <-  data[-TrainIDs,]

dataMod <- glm(hd ~ sex, data=Train, family ='binomial')
dataMod
summary(dataMod)

predict(dataMod, Test)
predict(dataMod, Test, type = 'response')

#### Checking the accuracy of the predictions agains the test set.
pH <- predict(dataMod, Test, type = 'response')
labelsH <- ifelse(pH> 0.5, 'Unhealthy', 'Healthy')
labelsH

table(labelsH == Test$hd)/length(Test$hd)
